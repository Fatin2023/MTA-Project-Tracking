...
app.get('/api/users', async (req, res) => {
    try {
        const result = await pool.query(`
            ...
            FROM users u LEFT JOIN members m ON u.member_id = m.id ORDER BY u.id
        `);
...
app.get('/api/assignments', async (req, res) => {
    try {
        ...
        res.json(result.rows.map(r => ({ id: r.id, projectId: r.project_id, memberId: r.member_id })));
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/assignments', async (req, res) => {
    ...
    try {
        await pool.query(`INSERT INTO project_assignments (project_id, member_id) VALUES ($1, $2)
            ON CONFLICT (project_id, member_id) DO NOTHING`, [projectId, memberId]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/attendance/:id', async (req, res) => {
    ...
    try {
        await pool.query('UPDATE attendance SET date = $1, clock_in = $2, clock_out = $3 WHERE id = $4',
